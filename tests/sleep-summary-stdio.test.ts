...
async function temporaryStore(): Promise<string> {
	const directory = await mkdtemp(join(tmpdir(), "mcp-whoop-sleep-summary-"));
 ...
 * on the failure without pinning down which shape the SDK chose.
 */
...
describe("the sleep summary over real stdio", () => {
	...
	it("refuses a days outside 1-30 without asking WHOOP anything", async () => {
		...
		const outcomes = await withClient(
			{ store, whoopBaseUrl: whoop.baseUrl },
			async (client) => [
	...
	it("hides get_sleep_summary when read:sleep was not granted, keeping list_cycles", async () => {
		const whoop = await startFakeWhoop([[]]);
		const store = await temporaryStore();
		...
		expect(names).not.toContain("get_sleep_summary");
