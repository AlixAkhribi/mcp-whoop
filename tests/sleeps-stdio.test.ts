...
async function temporaryStore(): Promise<string> {
	const directory = await mkdtemp(join(tmpdir(), "mcp-whoop-sleeps-"));
...
async function withClient<T>(
	...
	const client = new Client({ name: "sleeps-stdio-test", version: "0.0.0" });
...
describe("the sleep tools over real stdio", () => {
	...
	it("carries the cycle each sleep belongs to, and names it in the page schema", async () => {
		...
		const { declared, page } = await withClient(
			{ store, whoopBaseUrl: whoop.baseUrl },
			async (client) => ({
	...
	it("hides list_sleeps when read:sleep was not granted, keeping list_cycles", async () => {
		const whoop = await startFakeWhoop();
		const store = await temporaryStore();
		...
		expect(names).not.toContain("list_sleeps");
		expect(names).toContain("list_cycles");
	}, 30_000);

	it("hides get_cycle_sleep when read:sleep was not granted, keeping get_cycle", async () => {
		const whoop = await startFakeWhoop();
		const store = await temporaryStore();
		await seedStore(store, ["read:cycles", "offline"]);
		...
		expect(names).not.toContain("get_cycle_sleep");
		expect(names).toContain("get_cycle");
	}, 30_000);

	it("hides get_sleep when read:sleep was not granted, keeping list_cycles", async () => {
		const whoop = await startFakeWhoop();
		const store = await temporaryStore();
		await seedStore(store, ["read:cycles", "offline"]);
		...
		expect(names).not.toContain("get_sleep");
		expect(names).toContain("list_cycles");
	}, 30_000);
});
