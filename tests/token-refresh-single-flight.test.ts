...
import { storeLockPath } from "@/auth/tokens/lock";
...
async function temporaryStore(): Promise<string> {
	const directory = await mkdtemp(join(tmpdir(), "mcp-whoop-flight-"));
 ...
 * store and stand-in WHOOP. The client is handed back open, so a case can hold
 * several processes on one store at once; teardown closes them.
 */
...
describe("single-flight refresh across processes", () => {
	it("sends exactly one refresh when two processes race an expired token, and both calls succeed", async () => {
		...
		const first = await connectClient({ store, whoopBaseUrl: whoop.baseUrl });
		const second = await connectClient({ store, whoopBaseUrl: whoop.baseUrl });
