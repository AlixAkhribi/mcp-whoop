...
/** The four listed ahead of it, in the order a user's picker shows them. */
...
/**
 * The whole curated surface, entry for entry, in the one order `resources/list`
 * advertises it: the day first, the person it belongs to next, the body that
 * day was scored against after them, and the two weeks that explain the day
 * last — recovery, then the sleep beneath it.
 *
 * Asserted whole rather than entry by entry: never a sixth entry nobody
 * asked for, and never these five shuffled.
 */
const CANONICAL_RESOURCE_ORDER = [
	TODAY_URI,
	PROFILE_URI,
...
async function temporaryStore(): Promise<string> {
	const directory = await mkdtemp(
		join(tmpdir(), "mcp-whoop-sleep-last-week-resource-"),
	);
...
async function withClient<T>(
	...
	const client = new Client(
		{ name: "sleep-last-week-resource-stdio-test", version: "0.0.0" },
...
describe("the sleep digest as a resource, over real stdio", () => {
	...
	it("carries a zero-lifetime private cache hint on the resources/read result", async () => {
		...
		const result = await withClient({ store, whoopBaseUrl }, (client) =>
			client.readResource({ uri: SLEEP_LAST_WEEK_URI }),
		);
	...
	it("still lists whoop://sleep/last-week but refuses its read for a grant lacking read:sleep", async () => {
		const store = await temporaryStore();
		// Everything but the one scope the nightly listing takes: what this grant
		// cannot buy is a read, and nothing about the listing.
		await seedStore(
			store,
			ALL_READ_SCOPES.filter((scope) => scope !== "read:sleep"),
		);
		...
		const { uris, refusal } = await withClient(
			...
			async (client) => ({
				...
				refusal: await refusedRead(client, SLEEP_LAST_WEEK_URI),
			}),
		);
	...
	it("answers the same five resources in the same order from two fresh processes", async () => {
		const whoopBaseUrl = await startFakeWhoop();
		const store = await temporaryStore();
		await seedStore(store, ALL_READ_SCOPES);

		// Two separate server processes, spawned one after the other exactly as two
		// hosts would spawn them: the order is a property of the surface, not of one
		// process's luck, so it has to survive being asked again from scratch.
		const first = await withClient({ store, whoopBaseUrl }, async (client) =>
			(await client.listResources()).resources.map((resource) => resource.uri),
		);
		const second = await withClient({ store, whoopBaseUrl }, async (client) =>
			(await client.listResources()).resources.map((resource) => resource.uri),
		);

		expect(first).toEqual(CANONICAL_RESOURCE_ORDER);
		expect(second).toEqual(CANONICAL_RESOURCE_ORDER);
	}, 30_000);

	it("answers the same canonical listing under a narrowed grant", async () => {
		const store = await temporaryStore();
		// The three physiological scopes and neither of the two identity ones: a
		// grant that may read some of the five and not others — exactly the kind
		// of connection state the listing is forbidden to follow.
		await seedStore(store, ["read:cycles", "read:sleep", "read:recovery"]);

		const uris = await withClient(
			{ store, whoopBaseUrl: NO_WHOOP_BASE_URL },
			async (client) =>
		...
		// The grant gates reads, never the listing: a user who granted less sees
		// the same five entries in the same canonical order, and learns what
		// their login may not read from the read itself.
		expect(uris).toEqual(CANONICAL_RESOURCE_ORDER);
	}, 30_000);
});
